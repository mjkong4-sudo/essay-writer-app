      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-border py-16 text-center">
          ...
          <p className="mt-1 text-sm text-muted/40">
            {search
              ? "No matches found"
              : "Double-click any word in your essay to translate and save it here"}
          </p>
        </div>
      ) : (
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0 flex-1">
                <p className="font-serif text-base font-semibold">{entry.word}</p>
                <p className="mt-0.5 text-sm text-primary">{entry.translation}</p>
              </div>
              {deleteConfirm === entry.id ? (
                ...
                <button
                  ...
                  className="shrink-0 rounded p-1 text-muted transition-opacity hover:bg-red-50 hover:text-danger sm:opacity-0 sm:group-hover:opacity-100"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-4 w-4">
